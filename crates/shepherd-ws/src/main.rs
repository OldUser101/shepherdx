...
async fn _main(config: Config) -> Result<()> {
    ...
    let mqtt_robot_control = config.channel.robot_control.clone();
    mqtt_client
        .subscribe_raw("#", move |t, v| {
            let mqtt_sender = mqtt_sender.clone();
            let mqtt_log_handle = mqtt_log_handle.clone();
            let mqtt_robot_control = mqtt_robot_control.clone();
            async move {
                dispatch_mqtt_message(mqtt_sender, mqtt_log_handle, t, mqtt_robot_control, v).await
            }
        })
        .await?;
